/**
 * TENPO ONE デモデータ投入スクリプト
 *
 * 使い方:
 *   SUPABASE_URL と SUPABASE_SERVICE_ROLE_KEY を環境変数に設定して
 *   node scripts/seed.mjs
 *
 * デモ企業「株式会社TENPO ONE DEMO」(is_demo=true) と3店舗、
 * スタッフ・テーブル・メニュー・顧客・予約・注文・会計・勤怠・請求書・
 * 小口現金・売上30日分を投入する。全データはDBから取得され、画面への
 * ハードコードは行わない。
 */
...
function jstDate(daysAgo, hour = 12, minute = 0) {
  ...
  return new Date(`${y}T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00+09:00`);
}
const bd = (daysAgo) => jstDate(daysAgo).toLocaleDateString('sv-SE', { timeZone: 'Asia/Tokyo' });

async function createUser(email, name, kana, pin) {
  ...
  // profiles はトリガーで生成される。PINを設定
  await db.from('profiles').update({ pin_code: pin }).eq('id', data.user.id);
  return data.user.id;
}

async function main() {
  console.log('=== TENPO ONE デモデータ投入 ===');

  // 既存デモ企業の確認
  const { data: existing } = await db.from('organizations').select('id').eq('is_demo', true).limit(1);
  if (existing?.length) {
    console.error('デモ企業が既に存在します。再投入する場合は先に削除してください。');
    process.exit(1);
  }
  ...
  const stores = await insert('stores', storesDef.map((s) => ({
    ...s, organization_id: orgId, seat_count: 40, booking_enabled: true,
    description: '旬の食材を使った創作和食と厳選ワインのお店です。',
  })));
  await insert('store_settings', stores.map((s) => ({
  ...
  for (const [email, name, kana, role, storeIdx, pin] of usersDef) {
    const uid = await createUser(email, name, kana, pin);
    userIds[email] = uid;
    const [m] = await insert('memberships', [{ organization_id: orgId, profile_id: uid, role }]);
    if (storeIdx.length) {
      await insert('membership_stores', storeIdx.map((i, n) => ({
  ...
  for (const s of stores) {
    const [floor] = await insert('floors', [{ organization_id: orgId, store_id: s.id, name: '1F', sort_order: 1 }]);
    const defs = [];
    for (let i = 1; i <= 6; i++) {
      defs.push({ organization_id: orgId, store_id: s.id, floor_id: floor.id, name: `T${i}`, capacity_min: 2, capacity_max: 4, sort_order: i });
    }
    defs.push({ organization_id: orgId, store_id: s.id, floor_id: floor.id, name: 'C1', capacity_min: 1, capacity_max: 2, is_counter: true, sort_order: 7 });
    defs.push({ organization_id: orgId, store_id: s.id, floor_id: floor.id, name: '個室1', capacity_min: 4, capacity_max: 8, is_private_room: true, sort_order: 8 });
  ...
  for (const s of stores) {
    ...
    await insert('business_hours', hours);
  }
  ...
  const taxRates = await insert('tax_rates', [
    ...
    { organization_id: orgId, name: '軽減税率 8%（内税）', rate: 8, is_reduced: true, is_inclusive: true },
  ]);
  const tax10 = taxRates[0].id;
  ...
  const cats = await insert('menu_categories', catDefs.map(([name, color], i) => ({
  ...
  const menuItems = await insert('menu_items', menuDefs.map(([cat, name, price, cost], i) => ({
    organization_id: orgId,
    category_id: cats.find((c) => c.name === cat).id,
    name, price, cost, takeout_price: cat === 'ドリンク' ? null : price,
    item_type: cat === 'ドリンク' ? 'drink' : 'food',
    tax_rate_id: tax10, sort_order: i + 1,
  })));
  const courseCat = cats.find((c) => c.name === 'コース').id;
  const courses = await insert('menu_items', [
    { organization_id: orgId, category_id: courseCat, name: '季節のおまかせコース', price: 5500, cost: 2200, item_type: 'course', duration_minutes: 120, tax_rate_id: tax10, description: '前菜からデザートまで全7品', sort_order: 100 },
    { organization_id: orgId, category_id: courseCat, name: '贅沢和牛コース', price: 8800, cost: 3900, item_type: 'course', duration_minutes: 150, tax_rate_id: tax10, description: '和牛メインの全8品', sort_order: 101 },
  ]);
    ...
    for (let si = 0; si < stores.length; si++) {
      ...
      for (let o = 0; o < nOrders; o++) {
        ...
        for (let it = 0; it < nItems; it++) {
          ...
          const lineTax = lineTotal - Math.floor(lineTotal / 1.1);
        ...
        const [order] = await insert('orders', [{
          ...
          table_id: tablesByStore[si][randInt(0, 7)].id,
        ...
        // 会計済み注文に対応する顧客履歴を更新
        if (customer) {
          await db.rpc('recalc_customer_stats', { p_customer_id: customer.id });
        }
      }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    resRows.push({
      ...
      course_id: rand() < 0.3 ? courses[0].id : null,
  ...
  for (let i = 0; i < 6; i++) {
    ...
    await insert('reservation_tables', [{ reservation_id: r.id, table_id: tablesByStore[si][i % 6].id }]);
  }
  // キャンセル・無断キャンセルの履歴例
  await insert('reservations', [{
    ...
    guest_phone: customers[7].phone, status: 'no_show', source_id: srcPhone, created_via: 'phone', consent_accepted: true,
  }]);
  ...
  await insert('commission_rules', [{
    ...
    target_type: 'personal_sales', method: 'rate', rate: 2.0, basis: 'tax_excluded',
  }, {
    organization_id: orgId, name: '店舗月間目標達成ボーナス',
    target_type: 'store_target', method: 'fixed', fixed_amount: 10000, basis: 'tax_excluded',
  }]);

  // ---- 勘定科目・小口現金 ----
  const accounts = await insert('expense_accounts', [
  ...
  const vendors = await insert('vendors', [
    ...
    { organization_id: orgId, name: 'クリーンサービス関東', phone: '03-5555-3333', closing_day: 31, payment_day: 31, note: '清掃・リネン' },
    { organization_id: orgId, name: '厨房機器メンテ株式会社', phone: '045-555-4444', closing_day: 31, payment_day: 25, note: '厨房機器保守' },
  ]);
  ...
  for (let si = 0; si < stores.length; si++) {
    ...
    await insert('inventory_items', [
      { organization_id: orgId, store_id: stores[si].id, name: '生ビール樽 10L', item_kind: 'product', unit: '樽', current_quantity: randInt(2, 6), reorder_point: 2, avg_cost: 6500, menu_item_id: beer.id },
      { organization_id: orgId, store_id: stores[si].id, name: '米 10kg', item_kind: 'ingredient', unit: '袋', current_quantity: randInt(3, 8), reorder_point: 3, avg_cost: 4200 },
      { organization_id: orgId, store_id: stores[si].id, name: '醤油 1.8L', item_kind: 'ingredient', unit: '本', current_quantity: randInt(2, 10), reorder_point: 2, avg_cost: 780 },
      { organization_id: orgId, store_id: stores[si].id, name: 'おしぼり', item_kind: 'supply', unit: '箱', current_quantity: randInt(1, 5), reorder_point: 2, avg_cost: 1200 },
      { organization_id: orgId, store_id: stores[si].id, name: '割り箸', item_kind: 'supply', unit: '袋', current_quantity: randInt(5, 20), reorder_point: 5, avg_cost: 350 },
    ]);
  }
  ...
  await insert('customer_tag_links', [
