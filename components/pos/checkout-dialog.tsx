...
export function CheckoutDialog({
  ...
  /**
   * 支払方法をタッチしたときの動作（単一選択）。
   * タッチした支払方法だけが下に出る。別の方法をタッチすれば置き換わり、
   * 同じ方法をもう一度タッチすれば取り消す。
   * 足りないぶんが残っているときだけ、次にタッチした方法が残額として積まれる。
   */
      ...
      <div className="grid min-h-0 flex-1 gap-3 overflow-auto p-3 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,0.85fr)_470px] lg:overflow-hidden">
        ...
        <section className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-line bg-white">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-3">
            {/* 別々会計は金額のすぐ下（2026-09-25 店舗要望）。
                押すと伝票分割（品目ごと）へ進む。食べた分だけ先に会計して、残りは元の伝票に残る */}
            <div className="mb-3 border-b border-line pb-3">
              <button
                type="button"
                onClick={() => setSplitPick(splitPick != null ? null : {})}
              ...
              >
                別々会計
                <span
                  className={cn('text-[10px] font-semibold', splitPick != null ? 'text-white/80' : 'text-ink-3')}
                >
                  Separate payment
                </span>
              </button>
