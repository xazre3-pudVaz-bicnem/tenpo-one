    ...
    <div className="pb-6">
      <div className="space-y-2 px-3 pt-3 pb-2">
        <label className="relative block">
          ...
          <Search className="pointer-events-none absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-[#a393b5]" aria-hidden />
          <input
            type="search"
            value={query}
          ...
          {(
            ...
            <button
              ...
              className={cn(
                ...
                filter === value ? 'border-[#7b3fe4] bg-[#7b3fe4] text-white' : 'border-[#e3dbf1] bg-white text-[#5e4777]'
              )}
            >
              {label}
            </button>
        ...
        <p className="text-xs leading-relaxed text-[#8a769d]">
          売切にした商品は、レジ・ハンディ・お客様QRで注文できなくなります。入荷したら「販売再開」を押してください。
        </p>
      </div>

      {sections.length === 0 ? (
        <p className="px-6 py-10 text-center text-sm text-[#8a769d]">
          {filter === 'soldOut' && !query ? '売切中の商品はありません' : '該当する商品がありません'}
        </p>
      ) : (
        sections.map((section) => (
          <section key={section.id} className="mt-2">
            <h2 className="sticky top-0 z-10 bg-[#f6f3fb]/95 px-3 py-1.5 text-xs font-bold tracking-wide text-[#5e4777] backdrop-blur">
              {section.name}
            </h2>
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-[#8a769d]">
                        ...
                        {item.shared && <span className="rounded bg-[#efe9f7] px-1.5 py-0.5 text-[10px]">全店共通</span>}
                      </p>
                    </div>
                    <button
                      type="button"
