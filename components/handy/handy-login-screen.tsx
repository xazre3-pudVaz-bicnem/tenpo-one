...
export function HandyLoginScreen({
  ...
  return (
    <section
      ...
      className="fixed inset-0 z-40 flex flex-col overflow-y-auto bg-[#211c28] px-7 pt-[max(16px,env(safe-area-inset-top))] pb-7 text-white"
    >
      <div className="-mx-2.5 flex items-center gap-2.5 text-[11px]">
        <span
          className="inline-flex h-[25px] w-11 items-center justify-end rounded-[20px] bg-[#9f2c6c] p-0.5"
          aria-hidden
        >
        ...
        <button
          ...
          className="flex min-h-[35px] items-center gap-1 rounded border border-[#ffffff26] bg-[#ffffff16] px-2.5 text-[#e8dce4]"
        >
      ...
      <div className="mt-12 text-center">
        <span className="inline-flex items-center gap-[9px] text-[30px] font-extrabold tracking-[-0.4px]">
          ...
          <span className="text-[#d68ab8]">TENPO</span>
        ...
        <p className="mt-[22px] flex items-center justify-center gap-1.5 text-base">
          <CalendarDays className="h-4 w-4 text-[#e5c8d8]" aria-hidden />
          {today}
        </p>
      </div>
      ...
      <div className="my-auto grid grid-cols-[60px_1fr] items-center gap-[13px] py-9 text-[13px]">
        <span>店舗名</span>
        <span className="flex min-h-10 w-full items-center rounded-lg border border-[#e8dce4] bg-white px-2 text-base text-[#2a1f2e]">
        ...
        <select
          ...
          className="min-h-10 w-full rounded-lg border border-[#e8dce4] bg-white px-[7px] text-base text-[#2a1f2e]"
        >
          {clerks.map((c) => (
      ...
      <button
        ...
        onClick={login}
        disabled={pending}
        className="mt-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-[9px] bg-[#9f2c6c] text-[19px] font-semibold text-white shadow-[0_3px_10px_#9f2c6c1a] active:bg-[#7e2256] disabled:opacity-45"
      >
      ...
      <p className="mt-2.5 text-center text-[9px] leading-relaxed text-[#e5c8d8]">
        {clerks.length === 0
      ...
      {setupOpen && (
        ...
        >
          <div
            className="w-full max-w-[370px] rounded-xl bg-white p-5 text-[#2a1f2e] shadow-[0_20px_90px_#0005]"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="mb-3 text-lg font-bold">POS設定</h2>
            <dl className="text-sm">
              <div className="flex items-center justify-between gap-2 border-b border-[#efe6ec] py-2.5">
                <dt className="text-[#7f6e7a]">接続</dt>
                <dd className="font-bold text-[#1e6b4d]">接続済み</dd>
              </div>
              <div className="flex items-center justify-between gap-2 border-b border-[#efe6ec] py-2.5">
                <dt className="text-[#7f6e7a]">店舗</dt>
                <dd className="truncate font-bold text-[#4a3444]">{storeName}</dd>
              </div>
              <div className="flex items-center justify-between gap-2 border-b border-[#efe6ec] py-2.5">
                <dt className="text-[#7f6e7a]">端末</dt>
                <dd className="truncate text-[#4a3444]">{accountName}</dd>
              </div>
            </dl>
            <p className="mt-3 text-[11px] leading-relaxed text-[#7f6e7a]">
              別の店舗につなぎ直すときは、レジ（管理画面）の 設定 → ハンディ端末 でこの端末を解除し、
              新しい QR を読み取ってください。
            </p>
            <Link
              href="/app/dashboard"
              className="mt-2 flex min-h-[46px] items-center gap-2 border-b border-[#efe6ec] text-sm text-[#9f2c6c]"
            >
