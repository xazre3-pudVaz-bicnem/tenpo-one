...
import { createTestPrint, deletePrinterConfig } from '@/app/app/settings/printers/actions';
...
export function PrintersPanel({ storeId, initial }: { storeId: string; initial: PrinterConfigRow[] }) {
  ...
  const handleTestPrint = (row: PrinterConfigRow) => {
    startTransition(async () => {
      const result = await createTestPrint(row.id, storeId);
      if (result.error || !result.jobId) {
        toast(result.error ?? 'テスト印刷の作成に失敗しました', 'error');
        return;
      }
      window.open(`/app/settings/printers/test-print?job=${result.jobId}`, '_blank', 'noopener,noreferrer');
    });
  };
            ...
            <TBody>
              {rows.map((r) => (
                <Tr key={r.id}>
                  ...
                  <Td>{CONNECTION_LABEL[r.connectionType] ?? r.connectionType}</Td>
