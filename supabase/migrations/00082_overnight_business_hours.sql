-- =============================================================
-- 深夜営業（閉店が日付をまたぐ）に対応した予約枠（2026-09-27 Ronnie）
--
-- 画面では閉店・最終入店を 24:00〜30:00（翌朝6時）まで入れられる。DB の time 型には 24 を引いて保存する
-- （25:00 → 01:00）。なので「閉店（最終入店）が開店より前 ＝ 翌日」として扱う。
--
-- get_booking_availability:
--   これまで time 型のまま open〜last をループしていたため、最終入店が翌日（例 01:00）だと
--   1つも枠が出なかった。分（営業日の分＝開店から数えて 24h を超えてよい）でループし、
--   枠の時刻は '24:30' '25:00' のように営業日の表記で返す。
-- create_public_reservation:
--   p_time が '25:00' でも受け付ける（日付 + 分 で start_at を作る）。
-- それ以外は 00046 と同じ。
-- =============================================================
...
begin
  ...
  -- 営業日の分（開店から数える。閉店・最終入店が開店より前なら翌日 = +24h）
  v_open_min := (extract(hour from v_bh.open_time) * 60 + extract(minute from v_bh.open_time))::int;
  if v_bh.last_entry_time is not null then
    v_last_min := (extract(hour from v_bh.last_entry_time) * 60 + extract(minute from v_bh.last_entry_time))::int;
  else
    v_last_min := (extract(hour from v_bh.close_time) * 60 + extract(minute from v_bh.close_time))::int - 60;
  end if;
  if v_last_min < v_open_min then
    v_last_min := v_last_min + 24 * 60;
  end if;

  v_cutoff := now() + make_interval(mins => coalesce(v_settings.booking_cutoff_minutes, 120));
  v_step := greatest(5, coalesce(v_settings.slot_minutes, 30));
  v_day_start := (p_date::text || ' 00:00')::timestamp at time zone 'Asia/Tokyo';

  v_min := v_open_min;
  while v_min <= v_last_min loop
    v_slot_start := v_day_start + make_interval(mins => v_min);
    v_slot_end := v_slot_start + make_interval(mins => v_stay + v_buffer);
    -- 枠の表記は営業日の時刻（深夜は 24:30・25:00 のように 24 を超える）
    v_label := lpad((v_min / 60)::text, 2, '0') || ':' || lpad((v_min % 60)::text, 2, '0');
    ...
    if v_slot_start >= v_cutoff then
      ...
      if v_private > 0 then
        v_slots := v_slots || jsonb_build_object('time', v_label, 'available', false);
      else
...
-- create_public_reservation: p_time は '19:00' でも '25:00'（翌日 01:00）でも可。それ以外は 00046 と同一
...
begin
  ...
  -- 'HH:MM'（HH は 00〜30。24 以上は翌日の早朝）
  if p_time is null or p_time !~ '^\d{1,2}:\d{2}$' then raise exception 'INVALID_INPUT'; end if;
  v_time_min := split_part(p_time, ':', 1)::int * 60 + split_part(p_time, ':', 2)::int;
  if v_time_min < 0 or v_time_min > 30 * 60 then raise exception 'INVALID_INPUT'; end if;
  ...
  -- 日付の 0:00（日本時間）から分を足す（'25:00' → 翌日 01:00）
  v_start := ((p_date::text || ' 00:00')::timestamp at time zone 'Asia/Tokyo') + make_interval(mins => v_time_min);
