...
- **コース予約→POS引継（二重計上防止）**: 予約からPOS注文を作成する際、コースを**人数分1回だけ**計上し
  `reservations.course_posted_at` で二重計上を防止（再着席・注文再作成でも二重にならない）。migration 00044。E2E CASE8で検証。
- **モバイルオーダーからのコース二重課金防止**: コース(item_type=course)はQRメニュー(get_qr_menu=food/drink)に出ないため、
  お客様がモバイルからコースを再注文して二重課金することは構造的に発生しない。
- **stera 手動決済**: 会計時に金額表示→JT-C60で決済→成功確認後にTENPO ONEで確定（会計方法=クレジット）。
  既存POSの手動確定で成立（成功確認前にpaidにしない）。ネイティブAPI連携なし。

**任意の追加候補（急がない）**: モバイルオーダー画面に「ご予約コース」を参考表示、コース追加注文の可否を店舗設定化。
